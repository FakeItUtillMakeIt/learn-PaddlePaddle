...
DATA_DIR=os.path.join(CURRENT_DIR, "ml-1m")
MOVIE_FILE=os.path.join(DATA_DIR, "movies.dat")
RATING_FILE=os.path.join(DATA_DIR, "ratings.dat")
NEW_RATING_FILE=os.path.join(DATA_DIR,"new_rating.txt")
...
class MovieLen(object):
    def __init__(self, use_poster):
        ...
        # 声明每个数据文件的路径
        usr_info_path = USER_FILE
        if use_poster:
            rating_path = NEW_RATING_FILE
        else:
            rating_path = RATING_FILE

        movie_info_path = MOVIE_FILE
        self.poster_path = POSTER_DIR
        # 得到电影数据
