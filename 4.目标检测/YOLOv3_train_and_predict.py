...
from paddle.fluid.layers.detection import multiclass_nms as m_nms
...
def test_data_loader(datadir, batch_size= 10, test_image_size=608, mode='test'):
    ...
    """
    image_path=os.path.join(datadir,'images')
...
def draw_results(result, filename, draw_thresh=0.5):
    ...
    savepath=os.path.abspath(".") +'/test.jpg'
...
if __name__ == '__main__':
    ...
    TRAINDIR = os.path.join(CURRENT_DIR,'insects/train')
    TESTDIR =  os.path.join(CURRENT_DIR,'insects/test')
    VALIDDIR =  os.path.join(CURRENT_DIR,'insects/val')
    NMS_TOPK=400
    NMS_POSK=100
    NMS_THRESH=0.45
    ...
    if TRAIN:
        ...
        total_result=[]
        print("predict...")
        params_dict=paddle.load(os.path.join(CURRENT_DIR,"yolo_epoch0"))
        ...
        for i,data in enumerate(test_loader()):
            ...
            img_scale_data=paddle.to_tensor(img_scale_data)
            outputs=model.forward(img_data)
            bboxes,scores=model.get_pred(outputs,im_shape=img_scale_data,anchors=ANCHORS,
                                            anchor_masks=ANCHOR_MASKS)
            bboxes_data=bboxes.numpy()
            scores_data=scores.numpy()
            results=multiclass_nms(bboxes=bboxes_data,scores=scores_data,score_thresh=0.01,
                                        pre_nms_topk=NMS_TOPK,pos_nms_topk=NMS_POSK)
            for k in range(len(results)):
                results_k=results[k]
                img_name_k=os.path.join(os.path.join(TESTDIR,'images'),img_name[k])+'.jpeg'
                total_result.append([img_name_k,results_k.tolist()])
                
                draw_results(results_k,img_name_k,draw_thresh=0.2)
            print("processed {} pics".format(len(total_result)))
            
        
        
        json.dump(total_result,open("pred_results.json",'w'))
